 ...
 */
public class TreeCharSimplePlayer implements IPlayer {
    ...
    public TreeCharSimplePlayer(IBoard board, IRegion region) {
        uid = new UID();
    ...
    public void startPlay() {
        ...
        for(int i =0;i<cellsInRegion.size();i++){
            ...
            playCell(cell,new char[]{cell.getValue()});
    ...
    //        
    // }
    protected void playCell(ICell cell, char[] strSoFar) {
        if(BoggleMode.DEBUG_MODE){
        ...
        for (int i = 0; i < neighboors.size(); i++) {
            ...
            char[]nStr = new char[strSoFar.length+1];
            System.arraycopy(strSoFar, 0, nStr, 0, strSoFar.length);
            nStr[strSoFar.length]=nCell.getValue();
            //String nStr = strSoFar + nCell.getValue();
            // see if its in the dictionary
            
            SearchResult result = treeCharacter.findWord(nStr);
            //SearchResult result = treeCharacter.find(nodePath)
            if(BoggleMode.DEBUG_MODE){
            ...
            if ( result.getContains() ) {
                ...
                if ( !resultList.contains(nStr) ) {
                    resultList.add(new String(nStr));
                }
            }
