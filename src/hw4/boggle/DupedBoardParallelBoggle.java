...
public class DupedBoardParallelBoggle extends BaseParallelBoggle {
	
	private Thread[] threads;
	private IPlayer[] players;
	private IDictionary dictionary;
	
	public DupedBoardParallelBoggle(String wordListFile, int N, int numThreads) {
		super(wordListFile, N, numThreads);
		dictionary = createDictionary(wordListFile);
		BaseBoard.resetNextCellGetter();
	}

	@Override
	protected IBoard createBoard(int N, String[] board) {
		return new UnsynchronizedBoard(N, board);
	}
...
//		return super.doCreateDictionary(filename);
	...
	public void initializePlayers(IDictionary dictionary, int N, IBoard board) {
		int numThreads = threads.length;  
		for(int i = 0; i < numThreads; i++){
			players[i] = createPlayer(dictionary, createBoard(N,((UnsynchronizedBoard)board).getLetters()));
