...
public class TestMTWordInBoardPlayer {
	private static final String DICTIONARY = "./files/word.list";
	private static final String BOARD = "./files/board%dx%d.txt";
	private static final String BOARD_RESULTS_ST = "./output/board%dx%d.results.drc.st";
...
//    @Ignore("Takes too long")
	public void testWordInBoardPlayer() throws Exception {
		for(int i = 4; i < MAX_SIZE; i*=2) {
			System.out.println("Word to Board Single Threaded Approach "+i+"x"+i+":");
			...
			System.out.println("Word to Board Single Threaded Approach find took: " +(end- start)+"ns or "+((end- start)/1000000)+"ms");
			writeResultsToFile(player.getResults(), String.format(BOARD_RESULTS_ST,i,i));
			long endTotal = System.nanoTime();
			System.out.println("Word To Board Single Threaded Approach total took: " +(endTotal- startTotal)+"ns or "+((endTotal- startTotal)/1000000)+"ms\n\n");
		}
	}
	
	/**
	 * This method tests the WordListFinder.
	 * TODO: THIS WordInBoardPlayer doesn't work multi threaded there is a problem with
	 * the visiting a cell.  Threads sometiems think that they haven't already visited 
	 * a cell.
	 * 
	 */
...
//	@Test
	...
	public void testMTWordInBoardPlayer() throws Exception {
		for(int i = 4; i < MAX_SIZE; i*=2) {
			System.out.println("Word to Board Multi Threaded Approach "+i+"x"+i+":");
			long startTotal = System.nanoTime();
			List<String> words = readWordsFromFile(DICTIONARY);
			...
			for(int j = 0; j < 16; j++){
				 players[j] = new WordInBoardPlayer(board, words.subList(words.size()/16 * j, words.size()/16 * (j +1)));
			...
			long end = System.nanoTime();
			System.out.println("Multi Threaded Approach find took: " +(end- start)+"ns or "+((end- start)/1000000)+"ms");
			writeResultsToFile(players[1].getResults(), String.format(BOARD_RESULTS_MT,i,i));
			long endTotal = System.nanoTime();
			System.out.println("Multi Threaded Approach total took: " +(endTotal- startTotal)+"ns or "+((endTotal- startTotal)/1000000)+"ms");
		}
	}
	
	private static List<String> readWordsFromFile(String dictionaryFileName) throws Exception {
	...
	private static IBoard readBoardFromFile(String boardFileName) throws Exception {
	...
	private static void writeResultsToFile(List<String> results, String outputFileName) throws Exception {
